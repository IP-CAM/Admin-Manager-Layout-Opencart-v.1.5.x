<?php
	...
	//+--------------------------------------------------------------------------------------+
	//|                             Captura todos os Temas                                   |
	//+--------------------------------------------------------------------------------------+
	$listThemes = scandir(DIR_CATALOG . 'view/theme/');
	unset($listThemes[0]); //Deleta o .
	unset($listThemes[1]); //Deleta o ..
	...
	if (isset($this->request->get['file'])):
		...
		$arquivoSelecionado .= ' <span>('.$title[2].'.tpl)</span>';
	else:
		$arquivoSelecionado = 'Default: Account <span>(account.tpl)</span>';
	endif;
		...
		<div class="content">
			...
			<div class="headingThemeEditor">
				...
				<div class="right">
					...
					<select name="listThemes">
						<?php foreach ($listThemes as $theme): ?>
						<?php if (isset($name_theme) && $name_theme == $theme): ?>
					...
					<a onClick="openTheme()" class="button"><span><?php echo $btn_select; ?></span></a>
				</div>
			</div>
							...
							<?php foreach ($arquivos as $key => $values){ ?>
								<?php if ($key != 'name_theme'){ ?>
									<li>
										...
										<li>
											<a style="cursor:pointer" onClick="openFile('<?php echo $arquivos['name_theme'].'%2F'.$key.'%2F'.(basename($value, '.tpl').'%2F') ?>')">
...
<script><!--
	...
	function saveFile(){
		$.ajax({
			url: 'index.php?route=appaearance/themeditor/saveFile/&token=<?php echo $this->session->data['token'].'&file='.urlencode($_GET['file']).'&name_theme='.urlencode($_GET['name_theme']) ?>',
			type: 'post',
	...
	//Adiciona a classe de ativo na lista de arquivos
	/*$('#listFiles li').click(function (){
		$('.listFileActive').removeClass('listFileActive');
		$(this).addClass('listFileActive');
	});*/
	...
	function openFile(urlFile){
		...
		var filter_name_theme = $('select[name="listThemes"]').val();
		if (filter_name_theme) {
			url += '&name_theme=' + encodeURIComponent(filter_name_theme);
		}
		
		location = "index.php?route=appaearance/themeditor/&token=<?php echo $this->session->data['token'] ?>&file="+urlFile+url;
	};
	
	//Lista arquivos dos temas
	function openTheme(){
		location = 'index.php?route=appaearance/themeditor/&token=<?php echo $this->session->data['token'] ?>&name_theme='+$('select[name="listThemes"]').val();
	};
