...
<script><!--
	...
	function saveFile(){
		$.ajax({
			url: 'index.php?route=appaearance/modeleditor/saveFile/&token=<?php echo $this->session->data['token'].'&file='.urlencode($_GET['file']) ?>',
			type: 'post',
	...
	function openFile(urlFile){
		location = "index.php?route=appaearance/modeleditor/&token=<?php echo $this->session->data['token'] ?>&file="+urlFile+url;
	};
