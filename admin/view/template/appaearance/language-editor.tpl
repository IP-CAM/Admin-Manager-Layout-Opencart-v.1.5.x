<?php
	...
	//+--------------------------------------------------------------------------------------+
	//|                            Captura todos os Idiomas                                  |
	//+--------------------------------------------------------------------------------------+
	$listlanguages = scandir(DIR_CATALOG . 'language/');
	unset($listlanguages[0]); //Deleta o .
	unset($listlanguages[1]); //Deleta o ..
	...
	if (isset($this->request->get['file'])):
		...
		$arquivoSelecionado  = ucwords($title[0]).': ';
		$arquivoSelecionado .= ucwords(str_replace('_', ' ', $title[2]));
		$arquivoSelecionado .= ' <span>('.$title[2].'.php)</span>';
	else:
		$arquivoSelecionado = 'English: Account <span>(account.php)</span>';
	endif;
	...
	<div class="box">
		...
		<div class="content">
			...
			<div class="headinglanguageEditor">
				...
				<!---------------------------->
				<div class="right">
					<?php echo $entry_selectTheme; ?>
					<select name="listlanguages">
						<?php foreach ($listlanguages as $language): ?>
						<?php if (isset($name_language) && $name_language == $language): ?>
						<option value="<?php echo $language ?>" selected="selected"><?php echo ucwords($language) ?></option>
						<?php else: ?>
						<option value="<?php echo $language ?>"><?php echo ucwords($language) ?></option>
						<?php endif; ?>
						<?php endforeach; ?>
					</select>
					<a onClick="openlanguage()" class="button"><span><?php echo $btn_select; ?></span></a>
				</div>
			</div>
					...
					<div id="listFiles">
						<ul>
							<?php foreach ($arquivos as $key => $values){ ?>
								<?php if ($key != 'name_language'){ ?>
									<li>
										<h3><?php echo ucwords($key) ?></h3>
										<ul>
										<?php foreach ($values as $value){ ?>
										<li>
											<a style="cursor:pointer" onClick="openFile('<?php echo $arquivos['name_language'].'%2F'.$key.'%2F'.(basename($value, '.php').'%2F') ?>')">
												<?php echo ucwords(preg_replace('/[_-]/i', ' ', basename($value, '.php'))) ?><br/>
...
<script><!--
	...
	function saveFile(){
		$.ajax({
			url: 'index.php?route=appaearance/languageeditor/saveFile/&token=<?php echo $this->session->data['token'].'&file='.urlencode($_GET['file']).'&name_language='.urlencode($_GET['name_language']) ?>',
			type: 'post',
			...
			success: function (data){
				...
				if (data['sucesso']){
					$('#sucesso').php(data['sucesso']);
					$('#sucesso').fadeIn('fast');
				};
				
				if (data['erro']){
					$('#attention').php(data['erro']).fadeIn('slow');
				};
			}
		});
	}
	...
	function characterSpecial(){
		$('#windowFormatCode').load('view/javascript/editor/mode/caracterSpecial/caracterSpecial.php').dialog({
	...
	function addTable(){
		$('#windowFormatCode').load('view/javascript/editor/mode/table/table.php').dialog({
	...
	function autoFormatSelection(selectAll) {
		editor.setOption("mode", "phpmixed");
		if (selectAll)
	...
	function selectlanguage() {
		...
		editor.setOption("language", language);
	}
	...
	function openFile(urlFile){
		...
		var filter_name_language = $('select[name="listlanguages"]').val();
		if (filter_name_language) {
			url += '&name_language=' + encodeURIComponent(filter_name_language);
		}
		
		location = "index.php?route=appaearance/languageeditor&token=<?php echo $this->session->data['token'] ?>&file="+urlFile+url;
	};
	
	//Lista arquivos dos temas
	function openlanguage(){
		location = 'index.php?route=appaearance/languageeditor&token=<?php echo $this->session->data['token'] ?>&name_language='+$('select[name="listlanguages"]').val();
	};
