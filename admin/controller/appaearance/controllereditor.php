<?php
class ControllerAppaearanceControllereditor extends Controller {
	private $error = array(); 
	...
	public function index() {
		...
		if (isset($this->request->get['file'])):
			...
			$this->data['codFonte'] = $this->capturaCodFonte(DIR_CATALOG . 'controller/account/account.php');
		endif;
		...
		$this->template = 'appaearance/controller-editor.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function capturaLink(){
		...
		$array = $this->list_files( DIR_CATALOG . 'controller');
	...
	public function capturaCodFonte($filename = '', $folder = ''){
		...
		if (empty($theme) && empty($folder)):
			...
			$conteudo = file_get_contents(DIR_CATALOG . 'controller/' . $folder . '/'.$filename);
		endif;
	...
	public function saveFile(){
		...
		$handler = fopen(DIR_CATALOG . 'controller/'.$folder.'/'.$filename, 'w+');
