<?php
class ControllerAppaearanceThemeditor extends Controller {
	private $error = array(); 
	...
	public function index() {
		...
		if (isset($this->request->get['file'])):
			...
			$this->data['codFonte'] = $this->capturaCodFonte('../catalog/view/theme/default/template/checkout/cart.tpl');
		endif;
		
		//Verifica se o parâmetro NAME_THEME está sendo passado via URL | Caso exista captura o o link de todos arquivos .tpl do tema definido
		if (isset($this->request->get['name_theme'])):
			$this->data['arquivos'] = $this->capturaLink($this->request->get['name_theme']);
			$this->data['name_theme'] = $this->request->get['name_theme'];
		else:
		...
		$this->template = 'appaearance/theme-editor.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function capturaLink($theme = 'default'){
		
		$nArray = array();
		
		$nArray['name_theme'] = $theme;
		
		$array = $this->list_files( DIR_CATALOG . 'view/theme/' . $theme, 'tpl');
	...
	public function capturaCodFonte($filename = '', $theme = '', $folder = ''){
		// lê o conteúdo do arquivo para uma string
		if (empty($theme) && empty($folder)):
			$conteudo = file_get_contents($filename);
		else:
			$conteudo = file_get_contents(DIR_CATALOG . '/view/theme/'.$theme.'/template/'.$folder.'/'.$filename.'.tpl');
		endif;
	...
	public function saveFile(){
		$dados = explode('/', $this->request->get['file']);
		list($theme, $folder, $filename) = $dados;
		
		$handler = fopen('../catalog/view/theme/'.$theme.'/template/'.$folder.'/'.$filename.'.tpl', 'w+');
		
		if (fwrite($handler, html_entity_decode($this->request->post['code']))):
			echo json_encode(array('sucesso' => 'O arquivo <i><u>' . $theme . ': ' . $folder . ' <span>(' . $filename . '.tpl)</span></u></i> foi editado com sucesso!'));
		else:
			echo json_encode(array('erro' => 'Erro ao tentar editar o arquivo <i><u>' . $theme . ': ' . $folder . ' <span>(' . $filename . '.tpl)</span></u></i>!'));
		endif;
	}
